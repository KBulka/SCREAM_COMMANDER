...
void ARTSPawn::MoveTo(const float DeltaTime)
{
	...
	if (IsLocallyControlled())
	{
		...
		if (ReplicateCameraMovement)
		{
			ReplicateTransform(FVector(CurrentTargetVector.X, CurrentTargetVector.Y, GetActorLocation().Z));
		}
	}
}
...
void ARTSPawn::ReplicateTransform_Implementation(FVector Transform)
{
